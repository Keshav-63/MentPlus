...
const StudentDashboard = () => {
  ...
  const [availableGroupSessions, setAvailableGroupSessions] = useState([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      const [dashboardData, availableSessionsData] = await Promise.all([
        getStudentDashboardData(),
        getAvailableGroupSessions(),
      ]);
      ...
      setFilteredMentors(dashboardData.mentors || []);
      setMyBookings(dashboardData.myBookings || []);
      setMyGroupSessions(dashboardData.myGroupSessions || []);
      setMyTransactions(dashboardData.myTransactions || []);
      setUpcomingSessions(dashboardData.upcomingSessions || []);
      setOngoingSessions(dashboardData.ongoingSessions || []);
      setCompletedSessions(dashboardData.completedSessions || []);

      // Available group sessions
      setAvailableGroupSessions(availableSessionsData || []);
  ...
  }, [getStudentDashboardData, getAvailableGroupSessions]);
  ...
  const handleOpenGroupBookingModal = (session) => {
    console.log("handleOpenGroupBookingModal called with session:", session);
    if (!user) {
  ...
  const handleGroupPayment = async () => {
    try {
      const { order, bookingId } = await createGroupSessionOrder(
        selectedSession._id
      );
      const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY_ID,
        amount: order.amount,
        currency: order.currency,
        name: "MentorHub Group Session",
        description: `Join: ${selectedSession.title}`,
        order_id: order.id,
        handler: async function (response) {
          const data = {
            razorpay_payment_id: response.razorpay_payment_id,
          ...
          try {
            const result = await verifyGroupSessionPayment(data);
            toast.success(result.message);
      ...
      toast.error(
        useAuthStore.getState().error || "Could not initiate payment."
      );
    }
  };
  const domains = [
  ...
  return (
    <div className="min-h-screen py-20 px-4">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === "browse" && (
          <section className="browse-mentors" aria-label="browse mentors">
                        ...
                        {/* {availableGroupSessions.length > 0 ? (
                          availableGroupSessions.map((session) => (
                            <button
                              key={session._id}
                              onClick={() =>
                                handleOpenGroupBookingModal(session)
                              }
                            >
                              Join Session
                            </button>
            ...
            {/* --- NEW: Available Group Sessions Section --- */}
            <div className="mt-16">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {availableGroupSessions.length > 0 ? (
                  availableGroupSessions.map((session) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <button
                          onClick={() => handleOpenGroupBookingModal(session)}
                  ...
                  <p className="text-center text-gray-400 col-span-2">
                    No new group sessions are available to join right now.
                  </p>
                )}
              </div>
            </div>
      ...
      {isGroupBookingModalOpen && selectedSession && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="gradient-card p-8 rounded-2xl max-w-md w-full relative">
            <button
              onClick={() => setIsGroupBookingModalOpen(false)}
              className="absolute top-4 right-4 text-gray-400 hover:text-white"
            >
              <X />
            </button>
